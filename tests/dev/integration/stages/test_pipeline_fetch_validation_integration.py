...
def test_fetch_failure(mocker, user_service, caplog):
    ...
    mock_fetch = mocker.patch(
        "src.features.core_pipeline.stages.search_execution.fetch_web_results",
        return_value=None,
    )

    # Simulating the retry with validation block (steps 1 and 2)
    raw_search_data = retry_with_validation(
        mock_fetch, user_service, "Test Search Term"
    )
    ...
    mock_fetch.assert_called_with(
        user_service, "Test Search Term"
    )  # Ensure it's called with correct arguments
